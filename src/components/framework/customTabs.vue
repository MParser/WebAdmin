...
<template>
  <div class="custom-tabs border" :class="{ 'dark-mode': isDark }">
    ...
    <div class="tabs-header-wrapper">
      ...
      <!-- 标签容器 -->
      <div class="tabs-header" ref="tabsHeader" @wheel.prevent="handleWheel">
        <div
          class="tabs-nav-wrap"
          ref="tabsNav"
...
import { themeStore } from "@/store/sys/theme.js";
...
onMounted(() => {
  ...
  if (
    !tabs.value.find((tab) => tab.path === currentPath) &&
    currentRoute.name
  ) {
    store.addTabs({
...
const closeTab = async (path) => {
  ...
  if (activeTab.value === path) {
    ...
    const newPath =
      tabIndex > 0 ? tabs.value[tabIndex - 1].path : store.homeTab.path;
